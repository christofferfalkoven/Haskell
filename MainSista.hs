...
data Challo = Game {trunks1 :: (Float, Float), trunks2 :: (Float, Float), trunks3 :: (Float, Float),
              ...
              stateOfGame :: Int, extraLife :: (Int, Int), startScreen :: (Float, Float), endScreen :: (Float, Float)} deriving Show
...
initialState = Game {trunks1 = (300, 0), trunks2 = (450, 0), trunks3 = (600, 0), trunks4 = (750, 0),
                    ...
                    star1 = (675, 0), starColor = yellow, stateOfGame = 0, extraLife = (0, 0),
                    startScreen = ((-175),0), endScreen = (4910,5000)  }
...
updateFrame game = pictures [trunk1, trunk2, trunk3, trunk4, starPictures, player, walls, highscore, startwindow, endwindow]
    where
      ...
      highscore = pictures [translate 150 260 (color white (rectangleSolid 55 30)),
            translate 130 250 (scale 0.2 0.2 (color black (text (show (highScore game)))))]
...
collision game | (fst (trunks1 game)) <= (-260) && (fst (trunks1 game)) >= (-280) =
                if (((snd (trunks1 game)) + 10) > (playerLocation game) && ((snd (trunks1 game)) - 10) < (playerLocation game))
                then game
                else looseLife game
               | (fst (trunks2 game)) <= (-260) && (fst (trunks2 game)) >= (-280) =
                if (((snd (trunks2 game)) + 10) > (playerLocation game) && ((snd (trunks2 game)) - 10) < (playerLocation game))
                then game
                else looseLife game
               | (fst (trunks3 game)) <= (-260) && (fst (trunks3 game)) >= (-280) =
                if (((snd (trunks3 game)) + 10) > (playerLocation game) && ((snd (trunks3 game)) - 10) < (playerLocation game))
                then game
                else looseLife game
               | (fst (trunks4 game)) <= (-260) && (fst (trunks4 game)) >= (-280) =
                if (((snd (trunks4 game)) + 10) > (playerLocation game) && ((snd (trunks4 game)) - 10) < (playerLocation game))
                then game
                else looseLife game
